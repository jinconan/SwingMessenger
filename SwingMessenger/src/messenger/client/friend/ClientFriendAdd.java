...
/**********************************************************************
 * [클러이언트]친구추가하기
 * 기능 : 추가할 친구 아이디와 내 회원번호 전달. 서버는 클라이언트에게 검색 결과를 리스트에 담아서 전달. 
 * 
 * @author1 이정렬 [18/06/28]
 * [시나리오]
 * 1. 검색되어 조회된 아이디를 얻어서 나의 회원번호와 함께 Message<MemberVO>에 담아 보낸다.
 * 2. 서버로부터 받은 정보에 대하여 아래3,4번에 따라 움직이는 메소드를 작성한다
 * 3. Message<MemberVO>가 있을때, 추가가 완료되었다는 알림창을 띄운 후, 팝업창을 닫고,
 * 		친구목록을 다시 조회하여 리스트를 갱신한다.(ClientFriendList.java 재사용)
 * 4. 없을 경우, 추가를 실패했다는 안내와함께 반복될 경우 관리자에게 문의해달라는 안내를 한다.
 * 
 * cf. UI구성 후 서버로부터 받은 값을 올리는 것을 구현하도록 할 예정(06/29 프로젝트시간 중)
 * 
 **********************************************************************/
public class ClientFriendAdd {
	...
	/*	서버로부터 받은 메시지의 첫번째 자리에 들어있는 값은
		...
		switch(res.get(0)==null?0:1) {
			case 0://비어있어요
			/*	UI측에 안내용 팝업창을 띄웁니다.(이 UI는submenu창)
				"친구추가에 실패했습니다. 같은 문제가 반복될 경우 관리자에게 문의해주세요."*/
				//Insert Here
				System.out.println("List<MemberVO> res 는 비어있습니다.");
				break;
				
			case 1:
			/*	UI측에 안내용 팝업창을 띄웁니다.(이 UI는submenu창)
				"친구추가에 성공했습니다. 목록을 갱신합니다."
				라는 팝업창과 함께 SubMenu창을 닫고 친구목록을 갱신한다*/
				//도전과제.. 갱신된 목록에서 방금 추가된 회원이 선택되도록 커서?를 위치하여 보여주는건 어떨지?
				//Insert Here..
				System.out.println("List<MemberVO> res 는 차있는거같은데...");
